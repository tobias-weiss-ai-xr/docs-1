...
|------|-----|-----|---------|------|
| clrz14-06 | 8 cores | 32 GB | 500 GB SSD | K3s Master + etcd |
| clrz14-07 | 8 cores | 32 GB | 500 GB SSD | K3s Worker |
| clrz14-08 | 8 cores | 32 GB | 500 GB SSD | K3s Worker |
...
sudo hostnamectl set-hostname clrz14-06  # or 07, 08
...
Install K3s on master node (clrz14-06):
...
Install K3s on worker nodes (clrz14-07, clrz14-08):

```bash
# On worker nodes
curl -sfL https://get.k3s.io | K3S_URL=https://clrz14-06:6443 \
...
```bash
kubectl label nodes clrz14-06 \
  topology.kubernetes.io/region=rz03 \
  topology.kubernetes.io/zone=rz03-a \
  --overwrite

kubectl label nodes clrz14-07 \
  topology.kubernetes.io/region=rz03 \
  topology.kubernetes.io/zone=rz03-b \
  --overwrite

kubectl label nodes clrz14-08 \
  topology.kubernetes.io/region=rz03 \
  topology.kubernetes.io/zone=rz03-c \
  --overwrite
```
